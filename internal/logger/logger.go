...
func GetLogger(w io.Writer) *ZZZLogger {
	once.Do(func() {
		...
		instance = &ZZZLogger{output: colors.NewColorWriter(w)}
	})
	return instance
}
...
func (l *ZZZLogger) getLevelTag(level int) string {
	switch level {
	case levelFatal:
		return "FATAL   "
	case levelSuccess:
...
func (l *ZZZLogger) getColorLevel(level int) string {
	switch level {
	case levelCritical:
...
func (l *ZZZLogger) mustLog(level int, message string, args ...interface{}) {
	if level > logLevel {
		return
	}
	// Acquire the lock
...
func (l *ZZZLogger) mustLogDebug(message string, file string, line int, args ...interface{}) {
	if !debugMode {
		return
	}
